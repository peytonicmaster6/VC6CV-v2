...
int setupEGL(EGL_Setup *setup, SDL_Window* window)
{
	//EGLBoolean estatus;
	...
	CHECK_CONDITION(!setup->display, "eglGetDisplay() failed\n"); //check if EGL_NO_DISPLAY is returned or not
	...
	CHECK_CONDITION(!eglInitialize(setup->display, &setup->versionMajor, &setup->versionMinor),
	...
	EGLConfig config;
	EGLint numConfigs;
	CHECK_CONDITION(!eglChooseConfig(setup->display, configAttributes, &config, 1, &numConfigs),
					"Error: eglChooseConfig() failed\n");
	//CHECK_EVAL(estatus, "Failed to choose config!", error_surface);
	
	//EGLint vid;
    //CHECK_CONDITION(!eglGetConfigAttrib(setup->display, config, EGL_NATIVE_VISUAL_ID, &vid),
	//				"Error: eglGetConfigAtrib() failed\n");
    //CHECK_EVAL(estatus, "Failed to choose config attributes!", error_surface);
	   
	// Bind OpenGL ES API (Needed?)
	//estatus = eglBindAPI(EGL_OPENGL_ES_API);
	//CHECK_CONDITION(!eglBindAPI(EGL_OPENGL_ES_API), "Error: eglBindAPI() failed\n");
	...
	setup->context = eglCreateContext(setup->display, config, EGL_NO_CONTEXT, NULL);
	CHECK_CONDITION(!setup->context, "eglCreateContext() failed\n");
	
	setup->surface = eglCreateWindowSurface(setup->display, config, (EGLNativeWindowType)WMinfo.info.x11.window, 0); //gotta modify this?
	...
	//CHECK_CONDITION(!setup->surface, "eglCreatewindowSurface() failed\n");
	//CHECK_EVAL(setup->surface != EGL_NO_SURFACE, "Failed to create window surface!\n", error_surface);

	CHECK_CONDITION(!eglMakeCurrent(setup->display, setup->surface, setup->surface, setup->context),
	...
	if (eglerror != EGL_SUCCESS)
	{
		vcos_log_error("EGL error during init: error 0x%04x", eglerror);
		goto error_current;
	}
